...
`AnthropicProvider` maps Provider-neutral history to the native Messages contract: system text remains top-level, assistant calls become `tool_use`, tool outputs become leading `tool_result` blocks in the next user message, and image attachments become bounded base64 sources. Its SSE parser normalizes text, Thinking, partial tool JSON, and usage events. Model discovery uses the authenticated paginated `/models` endpoint. Remote HTTP is rejected unless the Provider configuration explicitly opts into unencrypted test traffic; HTTPS remains the default. Anthropic chat image input is separate from ImageX generation and editing, which this adapter does not expose.
